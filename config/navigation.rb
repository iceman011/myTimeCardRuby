...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    ...
    #
    if logged_in?
      primary.item :cards, 'MyCards', emp_path(current_emp) #emp_cards_path(current_emp) , if: proc { loggedin? }
    
      primary.item :emps, 'Employees', emps_path
      primary.item :task_types, 'Task Type', task_types_path
      primary.item :clm_work_items, 'CLM WorkItems', clm_work_items_path
      
      # Add an item which has a sub navigation (same params, but with block)
      #primary.item :lookups, 'Definition', "#" do |sub_nav| 
        # Add an item to the sub navigation (same params again)
      #  sub_nav.item :task_types, 'Task Type', task_types_path
      #  sub_nav.item :clm_work_items, 'CLM WorkItems', clm_work_items_path
    ...
    #primary.item :key_3, 'Admin', root_path, html: { class: 'special' }, if: -> { admin? }
