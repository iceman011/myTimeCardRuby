...
class removeFields {
  ...
  handleClick(link, e) {
    ...
    //get prev element [comment] which has nested form element id [card_entries_attributes_1_task_type_id]
    //if( link.previousElementSibling != null )
    //deleteField = fieldParent ? fieldParent.querySelector('input[type="hidden"][id="'+link.previousElementSibling.id+'"]') : null
    deleteField = $(link).prev("input[type=hidden]").val(1);
